...
### 무료 출시 + 선택적 개발자 후원
- 앱의 모든 비행계획 기능은 무료로 제공하고, 결제는 기능·콘텐츠·광고 제거·배지 등의 혜택이 없는 **자발적 개발자 후원**으로 운영
- 사용자 표기는 `개발자 후원` 또는 `개발자에게 커피 보내기`를 사용하고, 자선 모금으로 오해될 수 있는 `기부` 표현은 사용하지 않음

### 플랫폼별 결제 구조
- **iOS:** Apple StoreKit 소모성 인앱결제만 사용
  - 상품: 커피 / 커피+케이크 등 일회성 후원 상품
  - 인앱결제 플러그인이 없거나 결제가 실패해도 카카오페이 등 외부 결제로 우회하지 않음
  - 실패 시 `후원 상품을 불러오지 못했습니다` 또는 `상품 준비 중입니다` 안내만 표시
- **Android:** 추후 Android 정식 배포 시 혜택 없는 순수 후원 조건으로 카카오페이 외부 후원 링크 사용 가능
  - 후원 여부와 관계없이 앱 기능은 완전히 동일해야 함
  - 후원자 전용 기능·콘텐츠·배지·광고 제거 등 대가성 혜택을 제공하지 않음
  - 필요하면 Google Play Billing 일회성 상품으로 별도 구현할 수 있으나 Apple StoreKit 상품을 Android에서 공동 사용할 수는 없음
- **Web:** 카카오페이 등 외부 후원 링크 사용

### 공통 구현 원칙
- 화면의 후원 UI와 문구는 플랫폼 간 공유할 수 있지만 결제 엔진은 플랫폼별로 분기
- iOS에서 외부결제 링크가 절대 열리지 않도록 현재 KakaoPay fallback 제거가 배포 전 필수
- Android/웹에서는 카카오페이를 유지하되 iOS에서는 외부 링크를 실행하는 경로가 없도록 플랫폼 분기
- App Store Connect와 Google Play Console 상품은 각각 별도로 생성·관리하며 거래 및 영수증도 별개로 처리

### 코드 반영 완료
- `buyCoffee()`를 플랫폼별로 명시적으로 분리
  - iOS는 StoreKit `Iap` 플러그인만 호출하고, 플러그인/상품/결제 오류 시 화면에 안내만 표시
  - Android·웹에서만 카카오페이 후원 링크를 새 창으로 열도록 유지
- iOS 결제 실패 시 카카오페이로 이동하던 외부결제 fallback 제거
- iOS/Android 네이티브 자산 동기화 완료
- iOS 개발 빌드 `1.0 (1)`을 실제 `Kay phone`에 설치하고 실행 확인

### 배포 규정 작업 — 진행 중 일시정지
- App Store/TestFlight 배포 전 필수 항목을 재점검한 결과, 앱 안에 이용약관·개인정보처리방침·운항 안전 고지·문의 진입점이 없음을 확인
- SETUP 하단에 다음 메뉴와 공통 모달의 초안을 추가
  - 운항 안전 고지
  - 이용약관
  - 개인정보처리방침
  - 후원·문의 (`info@rufnekcrew.com`)
- 최초 실행 시 한 번 확인해야 하는 운항 안전 고지 초안을 추가
  - 공식 항법장비/공식 운항자료 대체 금지
  - 회사 규정·SOP·승인된 항공기 시스템·최신 차트·NOTAM·기상·ATC 지시 우선
  - OCR 및 계산 결과 독립 교차 확인
- 다음 TestFlight 후보를 위해 iOS 빌드 번호를 `1.0 (2)`로 준비
- 서비스워커 캐시를 `descent-planning-v65-cache-111`로 준비

> **중요 — 아직 배포 완료 상태가 아님**
> 사용자 요청으로 규정 작업을 여기서 일시정지했다. 공개 `privacy.html`/`terms.html` 작성, 개인정보 문구 최종 확정, 네이티브 재동기화, Release Archive, TestFlight 업로드는 진행하지 않았다. 현재 서비스워커 목록에는 아직 만들지 않은 두 정책 페이지 경로가 들어 있으므로 다음 세션에서 페이지를 작성하거나 참조를 정리한 뒤 배포해야 한다.

### Google Vision API 키 용도 정정
- `gcv_api_key`는 계정 인증이나 일반 사용자 추적용이 아니라, 과거 **접근 차트 자동 파싱용 Google Cloud Vision OCR**을 위해 사용하던 키임
- 현재 차트 업로드 UI는 수동입력 전환으로 숨겨져 있지만 관련 코드와 로컬 저장 키는 남아 있음
- 다음 규정 작업에서 개인정보처리방침을 확정할 때 다음 중 하나를 먼저 결정할 것
  1. 자동 파싱 기능을 다시 제공: 이미지가 Google Vision으로 전송되는 조건·목적·보관 범위를 정확히 고지
  2. 자동 파싱 기능을 폐기: Cloud Vision 코드와 API 키 저장 경로를 제거하고 정책에서도 제외

## 다음 할 일
- 차트 자동 파싱/Google Vision 기능 유지 여부를 사용자와 먼저 확인
- 공개 개인정보처리방침·이용약관 페이지 작성 및 앱 내 문구 최종 확정
- 운항 안전 고지 UI 검증 후 iOS/Android 네이티브 자산 재동기화
- iOS 후원 상품을 App Store Connect에 등록하고 Release Archive 생성
- 새 빌드를 TestFlight에 업로드한 뒤 빌드 번호와 규정 화면을 실제 설치본에서 확인
- Android 정식 배포 작업 시 이 섹션을 기준으로 카카오페이 후원 분기 구현 및 Google Play 정책 재확인
